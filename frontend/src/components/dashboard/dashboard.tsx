...
import { useRainfallIntelligence } from "@/hooks/use-rainfall-intelligence";
...
export function Dashboard() {
  ...
  const data = useRainfallIntelligence(current);
  const hierarchy =
    [
  ...
  const heading = location.selection.panchayat
    ? location.selection.panchayat.name
    : inDhanbad
      ? "Dhanbad District"
  ...
  const subheading = location.selection.panchayat
    ...
    : "ML-based rainfall downscaling for localized monsoon monitoring and agro-meteorological decision support.";
  ...
  return (
    <main>
      ...
      />
      <section
        className="border-b border-border bg-deep-panel px-4 py-4 text-header-foreground lg:px-6"
        aria-label="Rainfall intelligence summary"
      >
        <div className="mx-auto grid max-w-[1600px] gap-4 lg:grid-cols-[1.5fr_1fr] lg:items-center">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-xs leading-5 text-header-foreground/65">
              {subheading}
            </p>
          </div>
          <div className="grid grid-cols-[1fr_auto_1fr_auto_1fr] items-center border border-header-foreground/15 bg-header-foreground/5 p-3">
            <FlowStep label="Reference rainfall" />
            <ArrowDown className="size-3 -rotate-90 text-secondary" />
            <FlowStep label="Ridge Residual" />
            <ArrowDown className="size-3 -rotate-90 text-secondary" />
            <FlowStep label="Panchayat rainfall" />
          </div>
        </div>
      </section>
        ...
        <aside className="border-t border-border bg-deep-panel text-header-foreground xl:border-l xl:border-t-0">
          <div className="border-b border-header-foreground/15 p-5">
            <div className="flex items-center justify-between gap-3">
              <p className="text-[10px] font-bold uppercase text-secondary">Area briefing</p>
            ...
            <p className="mt-3 text-sm leading-6 text-header-foreground/70">
              {current.level === "panchayat"
                ? "Selected project Panchayat metadata record."
            ...
            <div className="mt-4 grid grid-cols-2 gap-px overflow-hidden rounded-sm bg-header-foreground/15">
              <Brief
                ...
                value={
                  current.level === "panchayat" && current.geometryStatus !== "boundary"
                    ? "Panchayat polygon unavailable"
                    : current.geometryStatus === "boundary"
                      ? "Verified boundary"
                      : current.geometryStatus === "point"
                        ? "Location point"
                        : "Metadata available"
                }
              />
              <Brief
                label={current.level === "panchayat" ? "GPCODE" : "Administrative code"}
                value={current.code}
              />
              <Brief
              ...
              <Brief
                label="Model status"
                value={data.state === "success" ? "Available" : "Data unavailable"}
              />
            </div>
          </div>
          <div className="border-b border-header-foreground/15 p-5">
            <div className="flex items-center justify-between gap-3">
              <p className="flex items-center gap-2 text-[10px] font-bold uppercase text-secondary">
                ...
                {point ? "Selected location" : "Map inspection"}
              </p>
              {point && (
                <button
          ...
          <div className="p-5">
            <p className="metric-label text-header-foreground/45">Model availability</p>
      ...
      <div className="mx-auto grid max-w-[1600px] gap-8 px-4 py-8 lg:px-6">
        ...
        <aside className="source-strip" aria-label="Data transparency">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="metric-label">Scientific data context</p>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">
                Administrative geometry uses geoBoundaries / DataMeet and LGD 2024. Panchayat
                records use the project Dhanbad metadata dataset keyed by GPCODE.
              </p>
            </div>
          </div>
