...
export function WeatherSummary({ data }: { data: RainfallIntelligence }) {
  const items = [
    ...
    },
    {
      ...
      note: "Panchayat downscaled",
      icon: CloudRain,
    },
    {
      label: "Downscaling difference",
      value:
        data.values?.differenceMm === undefined
          ? "—"
          : `${data.values.differenceMm > 0 ? "+" : ""}${data.values.differenceMm} mm`,
      note: "Versus reference",
      icon: GitCompareArrows,
    },
    {
      label: "Model status",
      value: data.state === "success" ? "Available" : "Unavailable",
      note: data.modelVersion,
      icon: Activity,
    },
  ];
  return (
    <section aria-labelledby="conditions-title">
      <div className="mb-3 flex items-end justify-between">
        <div>
          <p className="eyebrow">
            {data.values?.date
              ? `Historical rainfall · ${data.values.date}`
        ...
        <p className="max-w-56 text-right text-xs text-muted-foreground">
          {data.values?.date
            ? "Stored model evaluation record"
            : "Backend values appear when available"}
        </p>
      </div>
      <div className="grid grid-cols-2 overflow-hidden rounded-md border border-border bg-card sm:grid-cols-4">
        {items.map(({ label, value, note, icon: Icon }, index) => (
          <div
            className={`p-4 ${index > 0 ? "border-l border-border" : ""} ${index === 2 ? "max-sm:border-l-0" : ""} ${index > 1 ? "max-sm:border-t" : ""}`}
            key={label}
          >
